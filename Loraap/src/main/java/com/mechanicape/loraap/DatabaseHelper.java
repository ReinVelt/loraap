...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public LoraMessage getMessage(long Id)
    {
        LoraMessage message=new LoraMessage();
        String selectQuery ="SELECT  * FROM " +TABLE_RXPK +" WHERE id="+Long.toString(Id);
        Log.e(LOG, selectQuery);
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor c = db.rawQuery(selectQuery, null);
        // looping through all rows and adding to arr
        if (c.moveToFirst()) {
            do {
                ...
                int size=c.getInt(c.getColumnIndex(SIZE));;
                String data=new String(c.getBlob(c.getColumnIndex(DATA)));
                message=new LoraMessage(id,datum,tmst,chan,rfch,freq,stat,modu,datr,codr,lsnr,rssi,size,data);
            } while (c.moveToNext());
        }
        c.close();
        return message;
